...
impl Parse for u32 {
    fn parse(data: &[u8]) -> Result<Self> {
        ...
        let value = u32::from_be_bytes(value.try_into().expect("We've taken 4 bytes, this should be OK"));
        Ok((data, value))
    }
}
...
impl Parse for i32 {
    fn parse(data: &[u8]) -> Result<Self> {
        let (data, value) = bytes::take(4usize)(data)?;
        let value = i32::from_be_bytes(value.try_into().expect("We've taken 4 bytes, this should be OK"));
        Ok((data, value))
    }
}
...
impl Parse for u16 {
    fn parse(data: &[u8]) -> Result<Self> {
        ...
        let value = u16::from_be_bytes(value.try_into().expect("We've taken 2 bytes, this should be OK"));
        Ok((data, value))
    }
}
...
impl Parse for u8 {
    fn parse(data: &[u8]) -> Result<Self> {
        ...
        let value = u8::from_be_bytes(value.try_into().expect("We've taken 1 byte, this should be OK"));
        Ok((data, value))
    }
}
...
fn main() -> anyhow::Result<()> {
    ...
    match command.as_str() {
        ".dbinfo" => {
            ...
            let (_, header) = Header::parse(&header).map_err(|e| anyhow::Error::msg(format!("Error during parsing of header: {e:?}")))?;
